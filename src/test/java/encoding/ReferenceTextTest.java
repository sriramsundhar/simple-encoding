...
public class ReferenceTextTest {
    ...
    public void getRefrenceTextList() {
        ...
        assertEquals("First element of row 0 returns 1", "1", refrenceTextList.get(0).get(0));
        assertEquals("Second element of row 2 returns w", "w", refrenceTextList.get(1).get(1));
    }

    @Test
    public void getRefrenceTextMap() {
        assertEquals("Value of a is ", TextInfo.getInstance(2,0, null), referenceTextMap.get("a"));
        assertEquals("Value of v is ", TextInfo.getInstance(3,3, null), referenceTextMap.get("v"));
    }
}